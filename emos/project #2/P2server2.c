...
int recv_t(int socket, char* buf, int buff, int i) { //흐름 보기 위해 만든 함수
	...
	if (t > -1) {
		//printf("recv : %d, %d\n", i, t);//받은 것이 있으면 상태를 출력해준다(테스트 확인용).
	}
	return t;
}
int send_t(int socket, char* buf, int buff, int i) {//흐름 보기 위해 만든 함수
	...
	if (t > -1) {
		//printf("send : %d, %d\n", i, t); //보낸 것이 있으면 상태를 출력해준다(테스트 확인용).
	}
}
...
char* distingish(int a, char* ID, char* PW) { //로?戮?성공, 실패 메시지 출력 함수
	static char str[50] = "";
...
void* login_thread(void* argu) { //로그인함수
	...
	//printf("%d", c[val].waitFlag); 
		if (c[val].correctFlag > 0) { //로그인 성공시
			send_t(c[val].new_fd, "new", 4, 70); // i = 7 확인 신호를 보냄
			rcv_byte = recv_t(c[val].new_fd, buf, sizeof(buf), 80); //i = 8 확인신호를 받음
			send_t(c[val].new_fd, "chat", 4, 90);// i = 9 채팅모드 실행
			...
			send_t(c[val].new_fd, "new", 4, 170); // i = 7 확인신호를 보냄
			rcv_byte = recv_t(c[val].new_fd, buf, sizeof(buf), 180); //확인신호를 받음
			send_t(c[val].new_fd, "not", 4, 190); // i = 9 로그인 실패 신호 전송
		}
	while (1) {
...
int main(void) //accept문
{
	...
	while (1) {
		...
		//printf("their_addr : %d, new_fd : %d\n", their_addr.sin_addr.s_addr, their_addr);
